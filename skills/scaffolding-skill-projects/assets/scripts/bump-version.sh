...
# with drift detection and repo-wide audit.
#
# Usage:
#   bump-version.sh <new-version>   Bump all declared files
#   bump-version.sh --check         Report current versions (detect drift)
#   bump-version.sh --audit         Check + grep for old version strings
#
set -euo pipefail
...
cmd_audit() {
  ...
  if [[ "$found_undeclared" -eq 0 ]]; then
    ...
    echo ""
    echo "Review the above — add to .version-bump.json or audit.exclude as appropriate."
  fi
}
...
case "${1:-}" in
  --check) cmd_check ;;
  --audit) cmd_audit ;;
  --help|-h|"") echo "Usage: bump-version.sh <new-version> | --check | --audit"; exit 0 ;;
