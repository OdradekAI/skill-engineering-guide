...
**Announce at start:** "I'm using the iterating-skill-feedback skill to process your feedback and iterate on improvements."

## The Contract

This skill operates under a strict division of labor with `skill-forge:auditing-skill-projects`:
...
- After all changes are applied, this skill always invokes `skill-forge:auditing-skill-projects` to produce the post-change audit report

## Scope Clarification

This skill and `skill-forge:optimizing-skill-projects` serve different purposes:
...
Receive feedback
  ...
  → Version bump + doc update (if skill-project)
```
...
|--------|-----------|--------|
| Internal (skill-project) | Skill exists under `skills/` in current project | Modify in place |
...
- When modifying instructions, follow `skill-forge:writing-skill-content` guidelines
...
Invoke `skill-forge:auditing-skill-projects` to produce the post-change audit report. This is not optional — every iteration cycle ends with an audit.
...
For skills within a skill-project, after the audit confirms the changes are sound:

1. **Version bump** — Use `skill-forge:managing-skill-versions` to bump the appropriate version (patch for fixes, minor for new capabilities)
...
**Suggested by:**
- **skill-forge:auditing-skill-projects** — when audit findings indicate single-skill effectiveness issues (requires user confirmation)

**Calls:**
- **skill-forge:auditing-skill-projects** — automatic post-change audit
- **skill-forge:writing-skill-content** — reference for content modification guidelines
- **skill-forge:managing-skill-versions** — version bump after confirmed changes

**Cross-refers:**
- **skill-forge:optimizing-skill-projects** — when project-level engineering issues are discovered during iteration, suggest the user invoke optimizing instead
