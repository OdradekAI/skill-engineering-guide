...
description: "Use when reviewing a bundles repository for security risks, scanning third-party skills before installation, before publishing or releasing a bundles repository, after modifying hooks/plugins/scripts, when a user points to a skill folder/file or skill repository URL to review for safety, or when any executable code in a bundles repository needs safety verification — even code you wrote yourself, since accidental vulnerabilities are common"
---
...
Scan a bundles repository's attack surface — SKILL.md instructions, hook scripts, OpenCode plugins, agent prompts, and bundled scripts — for patterns that could exfiltrate data, destroy resources, install backdoors, or override safety controls.

**Core principle:** Trust but verify. Every bundles repository that ships executable code or agent instructions deserves a security scan, whether you wrote it or someone else did.

**This is a rigid skill.** Follow the scan process completely — do not skip categories or rationalize away findings.

**Announce at start:** "I'm using the scanning-security skill to scan this project for security risks."
...
- **Before release** — every bundles repository, every time
...
Hook scripts execute on every session start with the user's shell permissions. They are the highest-risk executable code in a bundles repository.
...
**Called by:**
- **bundles-forge:auditing** — Category 9: Security
- **bundles-forge:scaffolding** — post-scaffold security baseline

**Calls:**
- **bundles-forge:optimizing** — when security fixes overlap with optimization

**Pairs with:**
- **bundles-forge:adapting-platforms** — new platform adapters need security review
