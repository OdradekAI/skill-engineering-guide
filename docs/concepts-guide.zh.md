...
> **在 bundles-forge 中：** 8 个技能按中心辐射模型组织 — 3 个编排器（`blueprinting`、`optimizing`、`releasing`）派遣 4 个执行器（`scaffolding`、`authoring`、`auditing`、`testing`）+ 1 个引导元技能。编排器通过 `bundles-forge:<name>` 约定以文字指令派遣执行器。详见[在 bundles-forge 中的协作方式](#在-bundles-forge-中的协作方式)。

### Plugin（插件）

**[官方文档](https://code.claude.com/docs/en/plugins)** — 打包分发单元。

一个包含 `.claude-plugin/plugin.json`（清单）的目录，可组合技能、Agent、钩子、MCP 服务器、LSP 服务器、输出风格、`bin/` 可执行文件（插件启用期间加入 Shell 工具的 PATH），以及根级 `settings.json` 默认配置（目前支持 `agent` 键，可激活自定义子代理作为主线程）。插件对组件进行命名空间化（`/plugin-name:skill-name`）以避免冲突，通过 marketplace 分发。
...
> **在 bundles-forge 中：** 三个不可编辑子代理 — `inspector`、`auditor`、`evaluator` — 由技能派遣执行隔离的验证工作。它们设置了 `disallowedTools: Edit`，不能修改项目现有文件，但会将报告文件写入 `.bundles-forge/` 子目录（`audits/`、`evals/`、`blueprints/`）。
>
> **设计决策：** 用户始终通过技能交互，不直接调用 agent。技能在主对话中编排 agent 派遣，因为需要前置/后置逻辑（范围检测、报告合并）。子代理不可嵌套派遣其他子代理 — 所有编排权必须保留在技能层。
...
> **在 bundles-forge 中：** `session-start` 钩子（Bash 脚本，通过 `run-hook.cmd` 跨平台包装器调用）发送一条轻量提示，列出所有可用技能名称。完整路由上下文（`using-bundles-forge/SKILL.md`）在首次调用技能时按需加载。
...
> **在 bundles-forge 中：** 通过官方 Anthropic 市场分发（`/plugin install bundles-forge@bundles-forge-dev`）。
...
> **在 bundles-forge 中：** 提供 `bundles-forge` CLI 分发器，将子命令（`audit-skill`、`audit-security`、`audit-plugin` 等）路由到 `skills/auditing/scripts/` 和 `skills/releasing/scripts/` 中的脚本。
...
**在 bundles-forge 中：** 全部 8 个技能内联运行（它们需要与用户交互，以及派遣或被其他技能派遣）。3 个 agent 在 fork 上下文中运行（执行隔离诊断任务并返回报告）。
...
| **示例** | 单个代码审查技能 | bundles-forge（8 个技能组成中心辐射模型） |
...
bundles-forge 的三个子代理（`inspector`、`auditor`、`evaluator`）都设置了 `disallowedTools: Edit`。它们可以将报告文件写入 `.bundles-forge/` 子目录（`audits/`、`evals/`、`blueprints/`），但不能修改项目现有文件。这是刻意为之：

- **关注点分离** — agent 负责评估，skill 负责行动。一个能修复自己发现的问题的审计器会混淆角色。
- **信任边界** — 审计报告应当客观。如果审计器能修改项目文件，其结论可能被质疑（"它是不是偷偷修了问题才通过的？"）。
- **可预期性** — 用户调用 `bundles-forge:auditing` 时期望得到报告，而非代码被意外更改。
...
技能处理完整的交互生命周期：检测用户意图 → 派遣正确的 agent → 收集结果 → 呈现发现 → 提供下一步。在技能内部，**编排器**（`blueprinting`、`optimizing`、`releasing`）管理多步管线，而**执行器**（`scaffolding`、`authoring`、`auditing`、`testing`）执行专注的任务。子代理是**诊断工具** — 它们完成一项不可编辑的工作（将报告写入 `.bundles-forge/` 子目录，不修改现有文件）并返回摘要。

**具体示例 — 两阶段工作流审计：** `auditing` 技能同时需要 `auditor`（静态检查 W1-W9）和 `evaluator`（行为验证 W10-W11）。由于子代理不能派遣其他子代理，`auditing` 技能在主对话中按顺序派遣：Phase 1 派遣 `auditor`，等待报告返回；Phase 2 携带 Phase 1 的上下文派遣 `evaluator`。这种两阶段编排只有在技能（而非子代理）拥有工作流控制权时才可能实现。

### 为什么 session-start 只发送轻量提示而不注入完整技能上下文？

`session-start` 钩子发送一行约 180 字节的轻量提示，列出所有可用技能名称，而非注入完整的 `using-bundles-forge/SKILL.md`（约 6KB）。完整路由上下文在首次调用技能时按需加载。这一设计基于三个理由：

- **上下文窗口经济性** — 每次会话约 180 字节 vs 约 6KB。轻量提示为用户工作保留了上下文预算，同时仍提供足够的路由信息。
- **路由仍然准确** — 提示列出了所有技能名称，Agent 可以将用户意图匹配到正确的编排器或执行器。详细路由表仅在需要时加载。
- **容错性不变** — 如果钩子失败，Agent 仍然可以工作（用户可以通过 `bundles-forge:<skill-name>` 手动调用技能）。
...
1. **Marketplace** 分发插件 — 用户通过 `/plugin install bundles-forge@bundles-forge-dev` 安装
2. **Hook** 在会话启动时触发 — 发送轻量技能列表使 Agent 感知所有可用技能
3. **Skill** 执行工作 — 编排器管理管线并派遣执行器；执行器执行专注的任务。用户通过 `bundles-forge:<skill-name>`（如 `bundles-forge:auditing`）直接调用技能
4. **Subagent** 处理隔离任务 — 审计、检查、A/B 评估 — 并将报告写入 `.bundles-forge/` 子目录（`audits/`、`evals/`、`blueprints/`）
...
| [创作指南](authoring-guide.zh.md) | Skill 写作、agent 创作、质量清单 |
