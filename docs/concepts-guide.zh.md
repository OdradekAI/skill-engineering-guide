...
> **在 bundles-forge 中：** 8 个 `/bundles-*` 命令作为薄入口，重定向到对应的技能。
...
`session-start` 钩子发送一行约 120 字节的轻量提示，列出所有可用技能名称，而非注入完整的 `using-bundles-forge/SKILL.md`（约 6KB）。完整路由上下文在首次调用技能时按需加载。这一设计基于三个理由：

- **上下文窗口经济性** — 每次会话约 120 字节 vs 约 6KB。轻量提示为用户工作保留了上下文预算，同时仍提供足够的路由信息。
...
5. **Subagent** 处理隔离任务 — 审计、检查、A/B 评估 — 并将报告写入 `.bundles-forge/` 子目录（`audits/`、`evals/`、`blueprints/`）
