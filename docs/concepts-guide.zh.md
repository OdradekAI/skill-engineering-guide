...
> **在 bundles-forge 中：** `session-start.py` 钩子读取引导技能并注入 Agent 上下文，使其在每个会话开始时就获得所有可用技能的信息。使用 Python 实现以确保跨平台兼容性。
...
### 为什么 session-start.py 注入完整的技能清单？

`session-start.py` 钩子在每个会话开始时读取 `using-bundles-forge/SKILL.md` 并注入 Agent 上下文。替代方案是延迟加载 — 按需加载技能。但是：
