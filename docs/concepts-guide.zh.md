...
> **在 bundles-forge 中：** `session-start.py` 钩子发送一条轻量提示，列出所有可用技能名称。完整路由上下文（`using-bundles-forge/SKILL.md`）在首次调用技能时按需加载。使用 Python 实现以确保跨平台兼容性。
...
### 为什么 session-start.py 只发送轻量提示而不注入完整技能上下文？

`session-start.py` 钩子发送一行约 120 字节的轻量提示，列出所有可用技能名称，而非注入完整的 `using-bundles-forge/SKILL.md`（约 6KB）。完整路由上下文在首次调用技能时按需加载。这一设计基于三个理由：
