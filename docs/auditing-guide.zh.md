...
| 范围 | 适用场景 | 检查类别 | 脚本 |
|------|---------|---------|------|
| **完整项目** | 发布前、重大变更后、初次审查 | 10 大类、60+ 项检查 | `audit_plugin.py` |
| **单个技能** | 审查单个技能、评估第三方技能 | 4 类（结构、质量、交叉引用、安全） | `audit_skill.py` |
| **工作流** | 添加/移除技能后、链路集成检查 | 3 层（静态、语义、行为）、W1-W11 | `audit_workflow.py` |
| **仅安全扫描** | 快速基于模式的安全检查、安装前检查 | 7 类文件，已知危险模式 | `audit_security.py` |
...
```bash
python skills/auditing/scripts/audit_plugin.py <project-root>        # markdown 报告
python skills/auditing/scripts/audit_plugin.py --json <project-root>  # JSON 输出
```
...
| 9 | 文档 | 低 (1) | 文档一致性：`audit_docs.py`（D1-D9） |
...
```bash
python skills/auditing/scripts/audit_skill.py <skill-directory>           # markdown 报告
python skills/auditing/scripts/audit_skill.py <skill-directory>/SKILL.md   # 同样有效
python skills/auditing/scripts/audit_skill.py --json <skill-directory>     # JSON 输出
```
...
```bash
python skills/auditing/scripts/audit_skill.py <skill-directory>     # 质量 + 交叉引用 + 安全
python skills/auditing/scripts/audit_security.py <skill-directory>   # 安全扫描
```
...
2. **运行技能审计** — `python skills/auditing/scripts/audit_skill.py <downloaded-skill-dir>`
3. **与用户审查严重发现** 后再安装
...
```bash
python skills/auditing/scripts/audit_workflow.py <project-root>                          # 完整工作流审计
python skills/auditing/scripts/audit_workflow.py --focus-skills skill-a,skill-b <root>   # 聚焦模式
python skills/auditing/scripts/audit_workflow.py --json <project-root>                   # JSON 输出
```
...
2. 将技能添加到项目
3. 运行：python skills/auditing/scripts/audit_workflow.py --focus-skills new-skill-a,new-skill-b .
4. 修复焦点区域发现
5. 审查上下文发现以排除意外影响
```
...
```bash
python skills/auditing/scripts/audit_security.py <project-root>           # 项目级扫描
python skills/auditing/scripts/audit_security.py <skill-directory>         # 单技能扫描
python skills/auditing/scripts/audit_security.py --json <project-root>     # JSON 输出
```
...
python skills/auditing/scripts/audit_plugin.py --json . > audit.json   # 编排 lint + security + docs + workflow
