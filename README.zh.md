...
8 个技能覆盖 bundle-plugin 项目的完整生命周期，分为两层：

- **编排器**（`blueprinting`、`optimizing`、`releasing`）— 诊断、决策、委派。它们将多个技能串联起来完成多步目标。
- **执行器**（`scaffolding`、`authoring`、`auditing`、`testing`）— 单一职责工作者。可由用户直接调用，也可由编排器派遣。
...
flowchart LR
    ...
    Optimize -->|"委派变更"| Write
    Optimize --> Audit
    Audit -->|通过| Test["testing"]
    Test -->|通过| Release["releasing"]
```
...
| 测试 | `testing` | 动态验证 — 本地 dev-marketplace 安装、钩子烟雾测试、组件发现检测和跨平台就绪检查。 |
| 优化 | `optimizing` | 工程改进 — 描述触发准确性、token 效率、工作流重构、添加技能填补缺口、反馈迭代。 |
| 发布 | `releasing` | 编排发布前流水线：版本漂移检查、审计、测试、文档一致性检查、变更一致性审查、版本升级、CHANGELOG 更新和发布指引。 |

引导元技能 `using-bundles-forge` 通过钩子提供轻量会话提示，完整路由上下文通过 Skill 工具按需加载。
...
技能通过两种方式调用：**自动匹配**（Agent 将用户意图与技能的 `description` 字段匹配）或**显式引用**（通过 `bundles-forge:<skill-name>` 调用，如 `bundles-forge:auditing` 或 `bundles-forge:blueprinting`）。
...
| 范围 | 调用方式 / 脚本 | 检查内容 |
|------|----------------|---------|
| 完整项目 | `bundles-forge:auditing` 或 `bundles-forge audit-plugin` | 10 大类（结构、清单、版本同步、技能质量、交叉引用、工作流、钩子、测试、文档、安全） |
| 单个技能 | `bundles-forge:auditing` 指向 `skills/authoring` 或 `bundles-forge audit-skill` | 4 类（结构、技能质量、交叉引用、安全） |
| 工作流 | 显式请求 或 `bundles-forge audit-workflow` | 3 层：静态结构、语义接口、行为验证（W1-W11） |
| 仅安全扫描 | `bundles-forge:auditing`（security-only 模式）或 `bundles-forge audit-security` | 基于模式的检测，覆盖 7 类文件（技能内容、Hook 脚本、HTTP hooks、OpenCode 插件、Agent 提示词、打包脚本、MCP 配置） |
...
**审计之后：** 审计报告是纯诊断性的——它识别和评分问题，但不规定后续步骤。由用户或编排技能（如 `optimizing`、`releasing`）决定修复内容和方式。
...
<details>
<summary>技能执行链路与内部路由</summary>

> 概念解释见[概念指南](docs/concepts-guide.zh.md)。各技能详情见 [`docs/`](docs/) 中的指南。

### 技能执行

每个技能通过 description 自动发现或通过 `bundles-forge:<skill-name>` 显式调用。执行链路可以很深。

```mermaid
flowchart LR
    subgraph skills [入口技能]
        SK_BP["blueprinting"]
        SK_SF["scaffolding"]
        SK_AU["auditing"]
        SK_OP["optimizing"]
    ...
    SK_BP -->|"设计批准后"| scaffolding
    SK_BP -->|"内容编写"| authoring
    SK_BP -->|"初始审计"| SK_AU
    SK_AU -->|"发现问题"| SK_OP
    SK_OP -->|"委派变更"| authoring
    SK_OP -->|"验证修复"| SK_AU
    SK_RE -->|"发布前检查"| SK_AU
    SK_RE -->|"修复质量"| SK_OP
```

#### `bundles-forge:blueprinting` — 规划新 bundle-plugin
...
```
用户调用 bundles-forge:blueprinting
...
#### `bundles-forge:scaffolding` — 生成或适配项目结构
...
```
用户调用 bundles-forge:scaffolding
...
#### `bundles-forge:auditing`（security-only 模式）— 安全侧重审计

**适用场景：** 快速安全扫描。映射到 `auditing` 技能的 security-only 模式 — 仅运行第 10 类检查（7 大攻击面安全扫描：技能内容、Hook 脚本、HTTP hooks、OpenCode 插件、Agent 提示词、打包脚本、MCP 配置），跳过第 1-8 类。

#### `bundles-forge:optimizing` — 工程改进
...
#### `bundles-forge:releasing` — 版本升级与发布
...
- **显式调用技能**（`bundles-forge:auditing`、`bundles-forge:optimizing`）将 Agent 重新锚定到当前任务
